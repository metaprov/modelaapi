...
// validation
var _ webhook.Defaulter = &Meeting{}
...
// validation
var _ webhook.Validator = &Meeting{}
...
func (alert *Meeting) validate() error {
	...
	return apierrors.NewInvalid(schema.GroupKind{Group: "infra.modela.ai", Kind: "Meeting"}, alert.Name, allErrs)
}

func (alert *Meeting) validateMeta(fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList
	allErrs = append(allErrs, alert.validateName(fldPath.Child("name"))...)
	return allErrs
}

func (alert *Meeting) validateName(fldPath *field.Path) field.ErrorList {
	var allErrs field.ErrorList
...
func (task *Meeting) MarkDone() {
	task.CreateOrUpdateCond(MeetingCondition{
		Type:   MeetingDone,
		Status: corev1.ConditionTrue,
	})
	task.Status.Phase = MeetingPhaseDone
}
