...
type KnowledgeBaseSpec struct {
	...
	// MaxConcurrentReaders specifies how many documents can be read simultaneously. If unspecified, default to 1
	MaxConcurrentReaders *int32 `json:"maxConcurrentReaders,omitempty" protobuf:"varint,12,opt,name=maxConcurrentReaders"`
	// NodeParsers defines the collection of node parsers available to documents prior to indexing.
	...
	NodeParsers []NodeParserSpec `json:"nodeParsers,omitempty" protobuf:"bytes,13,opt,name=nodeParsers"`
	...
	DefaultNodeParser *string `json:"defaultNodeParser,omitempty" protobuf:"bytes,14,opt,name=defaultNodeParser"`
	// The configuration for Alerts generated by the Knowledge Base
...
type NodeParserSpec struct {
	...
	// The specification for a sentence window node parser
	...
	// The specification for a semantic node parser
	Semantic *SemanticNodeParserSpec `json:"semantic,omitempty" protobuf:"bytes,8,opt,name=semantic"`
}

// SemanticNodeParserSpec specifies a node parser which splits a document into groups of nodes that are semantically related
type SemanticNodeParserSpec struct {
	// EmbeddingModel specifies the embedding model to use for determining the similarity between nodes
...
type MixedNodeParserSpec struct {
	...
	// If a node parser cannot be found for a document, it will be skipped
...
type DocumentSpec struct {
	...
	// A document must be bound to at least one valid index
	...
	Flagged *bool `json:"flagged,omitempty" protobuf:"bytes,7,opt,name=flagged"`
...
type DatabaseReaderSpec struct {
	// The name of the connection to the database. The type of database which the connection references will determine the reader type
	ConnectionName string `json:"connectionName,omitempty" protobuf:"bytes,2,opt,name=connectionName"`
...
type RepositoryFileReader struct {
	...
	Extension *ExtensionFilter `json:"extension,omitempty" protobuf:"bytes,1,opt,name=extension"`
	// Filter files by their length
	Length *LengthFilter `json:"length,omitempty" protobuf:"bytes,2,opt,name=length"`
	// Filter files by their name
	Name *NameFilter `json:"name,omitempty" protobuf:"bytes,3,opt,name=name"`
	// Additional metadata to add to files matched by the reader
	Metadata map[string]string `json:"metadata,omitempty" protobuf:"bytes,4,opt,name=metadata"`
	// The file reader to use for the specified extensions
	Reader FileReaderSpec `json:"reader,omitempty" protobuf:"bytes,5,opt,name=reader"`
}

type RepositoryReaderSpec struct {
	// The name of the connection that provides authorization details for the repository.
	// The type of repository which the connection references will determine the reader type
	ConnectionName string `json:"connectionName,omitempty" protobuf:"bytes,2,opt,name=connectionName"`
	// UseDefaultReader indicates if all files in the repository be read, regardless if they are matched with a
	// reader specified in Readers. Files with extensions supported by Modela will be read by their respective reader
	// and files with unknown extensions will be read as plain-text.
	UseDefaultReader *bool `json:"useDefaultReader,omitempty" protobuf:"bytes,3,opt,name=useDefaultReader"`
	// The collection of filtered file readers, with each containing optional filters and a file reader specification.
	// Each reader is matched sequentially in the order that they are listed.
	//+kubebuilder:validation:Optional
...
type KnowledgeBaseStatus struct {
	...
	// FailureMessage is set to a failure message in the case that an error occurred during a refresh of the Knowledge Base
	...
	Documents []DocumentStatus `json:"documents,omitempty" protobuf:"bytes,6,opt,name=documents"`
	// Indexes contains the collection of statuses for each index defined by the Knowledge Base
	Indexes []IndexStatus `json:"indexes,omitempty" protobuf:"bytes,7,opt,name=indexes"`
