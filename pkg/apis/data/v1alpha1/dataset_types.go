...
type DatasetSpec struct {
	...
	// RunSpec specifies the configuration for runs created by the Dataset
	// +kubebuilder:validation:Required
	Run catalog.RunSpec `json:"run,omitempty" protobuf:"bytes,4,opt,name=run"`
	// Schedule specifies the schedule to create new runs for the Dataset
	// +kubebuilder:validation:Optional
	...
	// +kubebuilder:validation:Optional
	ModelClassRunName *string `json:"modelClassRunName,omitempty" protobuf:"bytes,28,opt,name=modelClassRunName"`
	// FeatureGroups contains the list of feature groups that were used to construct the dataset, if applicable
	// +kubebuilder:validation:Optional
	FeatureGroups []v1.ObjectReference `json:"featureGroups,omitempty" protobuf:"bytes,29,rep,name=featureGroups"`
...
type DatasetStatus struct {
	...
	// Version specifies the numerical generation of the object as tracked by Modela, which is
	// incremented each time the object is changed
	// +kubebuilder:default:=1
	// +kubebuilder:validation:Required
	// +required
	Version int32 `json:"version,omitempty" protobuf:"bytes,2,opt,name=version"`
	// RunVersion contains the version of the last run, which is incremented for each new run
	// +kubebuilder:default:=1
	RunVersion int32 `json:"runVersion,omitempty" protobuf:"bytes,3,opt,name=runVersion"`
	// LastRunAt specifies the time at which a run was last created for the Dataset
	//+kubebuilder:validation:Optional
	LastRunAt *metav1.Time `json:"lastRunAt,omitempty" protobuf:"bytes,4,opt,name=lastRunAt"`
	// LastFailureMessage specifies the failure message of the last run. If the last run succeeded, the field will be cleared
	//+kubebuilder:validation:Optional
	LastFailureMessage *string `json:"failureMessage,omitempty" protobuf:"bytes,5,opt,name=failureMessage"`
	...
	//+kubebuilder:validation:Optional
	Schedule catalog.RunScheduleStatus `json:"schedule,omitempty" protobuf:"bytes,7,opt,name=schedule"`
	// +patchMergeKey=type
	// +patchStrategy=merge
	// +kubebuilder:validation:Optional
