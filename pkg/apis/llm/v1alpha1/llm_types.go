...
type LLMSpec struct {
	...
	Models []LLMServiceSpec `json:"models,omitempty" protobuf:"bytes,5,opt,name=models"`
...
type ConcreteQueryEngine struct {
}
...
// LLMServiceSpec specifies the configuration for a single LLM
type LLMServiceSpec struct {
	// The LLM that will be served through the proxy at the specified Path
	LLM *ModelSpec `json:"model,omitempty" protobuf:"bytes,1,opt,name=model"`
	// The path suffix where the model will be served.
	// If unspecified, the API endpoint of the LLM provider will be used
	Path *string `json:"path,omitempty" protobuf:"bytes,2,opt,name=path"`
	// The API standard that will be served for the model.
	// If unspecified, default to the OpenAI API format
	Format *APIFormat `json:"format,omitempty" protobuf:"bytes,3,opt,name=format"`
}
