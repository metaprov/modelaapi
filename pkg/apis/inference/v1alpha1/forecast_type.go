...
const (
	...
	ForecastPhaseCreatingDataset   ForecastPhase = "CreatingDataset"
	ForecastPhaseWaitingForDataset ForecastPhase = "WaitingForDataset"
...
type ForecastSpec struct {
	...
	// PredictorRef is the name of the Predictor resource that will be used to evaluate forecasts for the
	...
	// If true, measurements for the metrics specified by the `Tests` field will be computed for each forecast and
	// stored in the Forecast's status with the average result of all forecasts
	// +kubebuilder:default:=false
	// Used usually for unit testing
	Labeled *bool `json:"labeled,omitempty" protobuf:"varint,3,opt,name=labeled"`
	// Indicates if the forecast is a forecast
	// +kubebuilder:default:=false
	// +kubebuilder:validation:Optional
	...
	// Tests specifies a collection of metrics that will be computed for each forecast
	// if the Labeled field of the Forecast is enabled
	// +kubebuilder:validation:Optional
	...
	// Resources specifies the resource requirements that will be allocated to the batch forecast Job
	// +kubebuilder:validation:Optional
	...
	// +kubebuilder:validation:Optional
	ForecastSpec ForecastForecastSpec `json:"forecastSpec,omitempty" protobuf:"bytes,16,opt,name=forecastSpec"`
	// CronForecastName specifies the name of the CronForecast that created the Forecast, if applicable
	// +kubebuilder:validation:MaxLength=63
	// +kubebuilder:default:=""
	// +kubebuilder:validation:Optional
	CronForecastName *string `json:"cronPredictorName,omitempty" protobuf:"bytes,17,opt,name=cronPredictorName"`
	// The reference to the ServingSite resource that hosts the Forecast
	// +kubebuilder:validation:Optional
...
type ForecastStatus struct {
	...
	// The collection of metrics that represent the average measurement across all forecasts for each
	...
	// The trigger that started the batch forecast
	//+kubebuilder:validation:Optional
	TriggeredBy catalog.TriggerType `json:"triggeredBy,omitempty" protobuf:"bytes,7,opt,name=triggeredBy"`
	// The location of logs produced by the batch forecast Job
	//+kubebuilder:validation:Optional
	...
	// In the case of failure, the Forecast resource controller will set this field with a failure reason
	//+kubebuilder:validation:Optional
	FailureReason *catalog.StatusError `json:"failureReason,omitempty" protobuf:"bytes,10,opt,name=failureReason"`
	// In the case of failure, the Forecast resource controller will set this field with a failure message
	//+kubebuilder:validation:Optional
	...
	// +patchStrategy=merge
	// +kubebuilder:validation:Optional
	Conditions []ForecastCondition `json:"conditions,omitempty" patchStrategy:"merge" patchMergeKey:"type" protobuf:"bytes,15,rep,name=conditions"`
}

// ForecastSpec specifies the details of a forecasting model
type ForecastForecastSpec struct {
