...
type ModelClassSpec struct {
	...
	// +kubebuilder:validation:Optional
	Entities []EntityRef `json:"entities,omitempty" protobuf:"bytes,6,opt,name=entities"`
	// The location where ModelClass artifacts (metadata, reports, and model artifacts) will be stored
	// +kubebuilder:validation:Optional
	ArtifactLocation data.DataLocation `json:"artifactLocation,omitempty" protobuf:"bytes,7,opt,name=artifactLocation"`
	...
	// +kubebuilder:validation:Optional
	Serving ModelClassServingSpec `json:"serving,omitempty" protobuf:"bytes,9,opt,name=serving"`
	...
	// +kubebuilder:validation:Optional
	ReportSchedule catalog.RunSchedule `json:"reportSchedule,omitempty" protobuf:"bytes,11,opt,name=reportSchedule"`
}

// Specification for the label information
type TrainingDataSpec struct {
	// The location of the training data. The training data can be a SQL statement or a file.
	// +kubebuilder:validation:Optional
	...
	// The training data primary key. This key will be used as unique id for each row in the training dataset
...
type ModelClassTrainingSpec struct {
	...
	// +kubebuilder:validation:Optional
	TrainingData TrainingDataSpec `json:"trainingData,omitempty" protobuf:"bytes,1,opt,name=trainingData"`
	...
	// +kubebuilder:validation:Optional
	LabRef v1.ObjectReference `json:"labRef,omitempty" protobuf:"bytes,2,opt,name=labRef"`
	...
	// +kubebuilder:validation:Optional
	ModelUnitTests catalog.TestSuite `json:"modelUnitTests,omitempty" protobuf:"bytes,4,opt,name=modelUnitTests"`
	// Training schedule
	// +kubebuilder:validation:Optional
	TrainingSchedule catalog.RunSchedule `json:"trainingSchedule,omitempty" protobuf:"bytes,5,opt,name=trainingSchedule"`
	// ModelImage specifies the configuration to upload Docker images of models to an image registry
	//+kubebuilder:validation:Optional
	ModelImage ModelImageSpec `json:"modelImage,omitempty" protobuf:"bytes,6,opt,name=modelImage"`
	// Promotion defines the way to promote models
	//+kubebuilder:validation:Optional
	PromotionPolicy catalog.PromotionType `json:"promotionPolicy,omitempty" protobuf:"bytes,7,opt,name=promotionPolicy"`
	// If true, retrain on drift
	// +kubebuilder:validation:Optional
	RetrainOnDrift *bool `json:"retrainOnDrift,omitempty" protobuf:"bytes,8,opt,name=retrainOnDrift"`
	// If true, retrain when there is a change in the labels data file. For example, if new labels were added
	// +kubebuilder:default:=true
	// +kubebuilder:validation:Optional
	RetrainOnLabelsDataChange *bool `json:"retrainOnLabelsDataChange,omitempty" protobuf:"bytes,9,opt,name=retrainOnLabelsDataChange"`
	...
	// +kubebuilder:default:=true
	// +kubebuilder:validation:Optional
	RetrainOnFeaturesMetadataChange *bool `json:"retrainOnFeaturesMetadataChange,omitempty" protobuf:"bytes,10,opt,name=retrainOnFeaturesMetadataChange"`
