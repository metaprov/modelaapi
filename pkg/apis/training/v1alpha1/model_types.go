...
type ModelSpec struct {
	...
	// +kubebuilder:validation:Optional
	Location *catalog.DataLocation `json:"location,omitempty" protobuf:"bytes,40,opt,name=location"`
...
type ModelStatus struct {
	...
	// +kubebuilder:validation:Optional
	TrainLabelEncoderURI string `json:"trainLabelEncoderURI,omitempty" protobuf:"bytes,28,opt,name=trainLabelEncoderURI"`
	// Label encoder that was trained on train data and tested on the test data
	// The URI of the label encoder binary file, if it exists
	// +kubebuilder:validation:Optional
	TestLabelEncoderURI string `json:"testLabelEncoderURI,omitempty" protobuf:"bytes,29,opt,name=testLabelEncoderURI"`
	// Label encoder that was trained on the full data
	// The URI of the label encoder binary file, if it exists
	// +kubebuilder:validation:Optional
	FullLabelEncoderURI string `json:"fullLabelEncoderURI,omitempty" protobuf:"bytes,30,opt,name=fullLabelEncoderURI"`
	// The URI of the logs file
	// +kubebuilder:validation:Optional
	LogsURI string `json:"logsURI,omitempty" protobuf:"bytes,31,opt,name=logsURI"`
	// The URI of the model profile, which contains visualizations produced during the profiling phase
	// +kubebuilder:validation:Optional
	ProfileURI string `json:"profileURI" protobuf:"bytes,32,opt,name=profileURI"`
	// The URI to the misclassification file produced during the testing phase
	// +kubebuilder:validation:Optional
	MisclassificationURI string `json:"misclassificationURI" protobuf:"bytes,33,opt,name=misclassificationURI"`
	// The URI to the model tarbell file
	// +kubebuilder:validation:Optional
	TarURI string `json:"tarURI" protobuf:"bytes,34,opt,name=tarURI"`
	// The URI to the model application file
	// +kubebuilder:validation:Optional
	AppURI string `json:"appURI" protobuf:"bytes,35,opt,name=appURI"`
	// The name of the Docker image produced by the Model
	// +kubebuilder:validation:Optional
	ImageName string `json:"imageName" protobuf:"bytes,36,opt,name=imageName"`
	...
	// +kubebuilder:validation:Optional
	ImpurityImportance []FeatureImportance `json:"impurityImportance,,omitempty" protobuf:"bytes,37,rep,name=impurityImportance"`
	// The collection of features and their importance, sorted by the greatest importance first
	// The collection is measured using permutation importance
	// +kubebuilder:validation:Optional
	PermutationImportance []FeatureImportance `json:"permutationImportance,omitempty" protobuf:"bytes,38,rep,name=permutationImportance"`
	// The URI of the model forecast
	// +kubebuilder:validation:Optional
	ForecastURI string `json:"forecastURI,omitempty" protobuf:"bytes,39,opt,name=forecastURI"`
	// The runtime training details.
	// +kubebuilder:validation:Optional
	Runtime RuntimeStatus `json:"runtime,omitempty" protobuf:"bytes,40,opt,name=runtime"`
	// TrainDatasetLocation specifies the location of the training dataset
	// +kubebuilder:validation:Optional
	TrainDatasetLocation catalog.FileLocation `json:"trainDatasetLocation,omitempty" protobuf:"bytes,41,opt,name=trainDatasetLocation"`
	// TestDatasetLocation specifies the location of the testing dataset
	// +kubebuilder:validation:Optional
	TestDatasetLocation catalog.FileLocation `json:"testDatasetLocation,omitempty" protobuf:"bytes,42,opt,name=testDatasetLocation"`
	// ValidationDataset specifies the location of the validation dataset
	// +kubebuilder:validation:Optional
	ValidationDataset catalog.FileLocation `json:"validationDatasetLocation,omitempty" protobuf:"bytes,43,opt,name=validationDatasetLocation"`
	// ObservedGeneration is the last generation that was acted on
	//+kubebuilder:validation:Optional
	ObservedGeneration int64 `json:"observedGeneration,omitempty" protobuf:"varint,44,opt,name=observedGeneration"`
	// The number of rows in the training dataset
	// +kubebuilder:validation:Optional
	TrainingRows int32 `json:"trainingRows" protobuf:"varint,45,opt,name=trainingRows"`
	// The number of rows in the testing dataset
	// +kubebuilder:validation:Optional
	TestingRows int32 `json:"testingRows" protobuf:"varint,46,opt,name=testingRows"`
	// The number of rows in the validation dataset, if applicable
	// +kubebuilder:validation:Optional
	ValidationRows int32 `json:"validationRows" protobuf:"varint,47,opt,name=validationRows"`
	// In the case of failure, the Model resource controller will set this field with a failure reason
	//+kubebuilder:validation:Optional
	FailureReason *catalog.StatusError `json:"failureReason,omitempty" protobuf:"bytes,48,opt,name=failureReason"`
	// In the case of failure, the Model resource controller will set this field with a failure message
	//+kubebuilder:validation:Optional
	FailureMessage *string `json:"failureMessage,omitempty" protobuf:"bytes,49,opt,name=failureMessage"`
	// The progress percentage of the Model, which is derived from the Model's current phase
	// +kubebuilder:validation:Optional
	Progress int32 `json:"progress,omitempty" protobuf:"varint,50,opt,name=progress"`
	// The size of the model binary in bytes
	// +kubebuilder:validation:Optional
	SizeInBytes int32 `json:"sizeInBytes,omitempty" protobuf:"varint,51,opt,name=sizeInBytes"`
	// The measured prediction latency
	// +kubebuilder:validation:Optional
	Latency float64 `json:"latency,omitempty" protobuf:"bytes,52,opt,name=latency"`
	...
	// The time at which the Model was set to release
	// +kubebuilder:validation:Optional
	PredictedAt *metav1.Time `json:"predictedAt,omitempty" protobuf:"bytes,56,opt,name=predictedAt"`
	// Sha256 of the model tar file
	// +kubebuilder:validation:Optional
	TarFileHash string `json:"tarFileHash,omitempty" protobuf:"bytes,57,opt,name=tarFileHash"`
	// Sha256 of the model image
	// +kubebuilder:validation:Optional
	ImageHash string `json:"imageHash,omitempty" protobuf:"bytes,58,opt,name=imageHash"`
	...
	// +kubebuilder:validation:Optional
	RocCurve catalog.RocAucCurve `json:"rocCurve,omitempty" protobuf:"bytes,66,opt,name=rocCurve"`
	// The Precision/Recall curve of the trained model
	// +kubebuilder:validation:Optional
	PRCurve catalog.PRCurve `json:"prCurve,omitempty" protobuf:"bytes,67,opt,name=prCurve"`
	// The confusion matrix of the train data.
	// +kubebuilder:validation:Optional
	TrainConfusionMatrix catalog.ConfusionMatrix `json:"trainConfusionMatrix,omitempty" protobuf:"bytes,68,opt,name=trainConfusionMatrix"`
	// The confusion matrix of the test data
	// +kubebuilder:validation:Optional
	TestConfusionMatrix catalog.ConfusionMatrix `json:"testConfusionMatrix,omitempty" protobuf:"bytes,69,opt,name=testConfusionMatrix"`
	// The collection of correlations of the features of the training dataset and the target feature
	// +kubebuilder:validation:Optional
	CorrelationsWithTarget []data.Correlation `json:"correlationsWithTarget,omitempty" protobuf:"bytes,70,rep,name=correlationsWithTarget"`
	// The top correlations between features of the training dataset
	// +kubebuilder:validation:Optional
	TopCorrelations []data.Correlation `json:"topCorrelations,omitempty" protobuf:"bytes,71,rep,name=topCorrelations"`
	...
	// +kubebuilder:validation:Optional
	Images catalog.Images `json:"images,omitempty" protobuf:"bytes,75,opt,name=images"`
	// The result of running the unit tests
	// +kubebuilder:validation:Optional
	UnitTestsResult catalog.TestSuiteResult `json:"unitTestsResult,omitempty" protobuf:"bytes,76,opt,name=unitTestsResult"`
	// The result of running the feedback unit tests, the feedback unit tests
	// +kubebuilder:validation:Optional
	FeedbackTestsResult catalog.TestSuiteResult `json:"feedbackTestsResult,omitempty" protobuf:"bytes,77,opt,name=feedbackTestsResult"`
	// The sub models uri file contain the results of running the sub model
	// +kubebuilder:validation:Optional
	GroupBy ModelGroupByStatus `json:"groupby,omitempty" protobuf:"bytes,78,opt,name=groupby"`
