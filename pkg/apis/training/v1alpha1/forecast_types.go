...
const (
	...
	/*
		TSFeatureAggAutocorrelation TSFeature   // agg_autocorrelation(x, param)	Descriptive statistics on the autocorrelation of the time series.
		TSFeatureAggLinearTrend	   // agg_linear_trend(x, param)	Calculates a linear least-squares regression for values of the time series that were aggregated over chunks versus the sequence from 0 up to the number of chunks minus one.
		TSFeatureApproximateEntropy	   // approximate_entropy(x, m, r)	Implements a vectorized Approximate entropy algorithm.
		TSFeatureArCoefficient	   // ar_coefficient(x, param)	This feature calculator fits the unconditional maximum likelihood of an autoregressive AR(k) process.
		TSFeatureAugmentedDickeyFuller	   // augmented_dickey_fuller(x, param)	Does the time series have a unit root?
		TSFeatureAutoCorrelation	   // autocorrelation(x, lag)	Calculates the autocorrelation of the specified lag, according to the formula [1]


		//binned_entropy	   // binned_entropy(x, max_bins)	First bins the values of x into max_bins equidistant bins.
